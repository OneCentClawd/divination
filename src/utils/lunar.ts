...
export function solarToLunar(year: number, month: number, day: number) {
  ...
  const ganZhiYear = HEAVENLY_STEMS[stemIndex] + EARTHLY_BRANCHES[branchIndex] + '年'

  // 月份和日期
  const monthStr = (isLeap ? '閏' : '') + CHINESE_MONTHS[lunarMonth - 1] + '月'
  const dayStr = CHINESE_DAYS[lunarDay - 1]
